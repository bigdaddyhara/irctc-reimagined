...
const pastTrips = [
  { id: 'trip-1', route: 'Mumbai Central → Pune', train: 'Deccan Queen · 12124', date: '18 Aug 2026', className: 'AC Chair Car', fare: 460, status: 'Completed' },
  { id: 'trip-2', route: 'New Delhi → Jaipur', train: 'Ajmer Shatabdi · 12015', date: '02 Aug 2026', className: 'AC Chair Car', fare: 780, status: 'Completed' },
  { id: 'trip-3', route: 'Chennai Central → Bengaluru', train: 'Brindavan Express · 12639', date: '21 Jul 2026', className: 'AC 3 Tier', fare: 620, status: 'Completed' },
]
...
function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('irctc-theme') === 'dark')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('irctc-theme', darkMode ? 'dark' : 'light')
  }, [darkMode])
  ...
  return <div className={`app-shell ${easyMode ? 'easy-mode' : ''}`}>
    ...
    <div className="main-column">
      <header className="topbar site-header">
        <div className="site-header-inner"><Brand onNavigate={() => goTo('home')} /><PrimaryNavigation activeView={view} onNavigate={handleNavigate} copy={copy} /><div className="header-actions"><button className="header-help" type="button" onClick={() => setToast('Help centre preview coming next.')}><Info size={15} aria-hidden="true" />Help</button><button className="theme-toggle" type="button" aria-label={darkMode ? 'Use light mode' : 'Use dark mode'} title={darkMode ? 'Use light mode' : 'Use dark mode'} onClick={() => setDarkMode((current) => !current)}>{darkMode ? <Sun size={17} aria-hidden="true" /> : <Moon size={17} aria-hidden="true" />}</button>{session ? <button className="profile-chip" type="button" onClick={() => goTo('profile')}><span className="avatar">{session.user.name.slice(0, 2).toUpperCase()}</span><span className="profile-name">{session.user.name}</span><ChevronRight size={15} aria-hidden="true" /></button> : <button className="profile-chip guest-profile" type="button" aria-label="Profile - log in" onClick={() => handleNavigate('profile')}><CircleUserRound size={18} aria-hidden="true" /><span className="profile-name">Profile</span></button>}</div></div>
      </header>
      <main id="main-content" className="content-area">
        ...
        {view === 'profile' && <ProfileView copy={copy} easyMode={easyMode} darkMode={darkMode} language={language} userName={session?.user.name} serviceRequests={serviceRequests} onEasyMode={() => { setEasyMode((current) => !current); setToast(!easyMode ? 'Easy Mode is on.' : 'Easy Mode is off.') }} onDarkMode={() => setDarkMode((current) => !current)} onLanguage={chooseLanguage} onLogout={() => { logoutDemo(); setSession(null); setServiceRequests(getServiceHistory('guest')); setToast('You are logged out of the demo.') }} />}
      </main>
    </div>
...
function ProfileView({ copy, easyMode, darkMode, language, userName, serviceRequests, onEasyMode, onDarkMode, onLanguage, onLogout }: { copy: Record<string, string>; easyMode: boolean; darkMode: boolean; language: Language; userName?: string; serviceRequests: ServiceRequest[]; onEasyMode: () => void; onDarkMode: () => void; onLanguage: (language: Language) => void; onLogout: () => void }) {
  return <><div className="eyebrow"><Accessibility size={14} aria-hidden="true" />{copy.profile}</div><h1>{copy.profileTitle}</h1><p className="lede">{copy.profileLede}</p>{userName ? <div className="signed-in-card"><span className="avatar">{userName.slice(0, 2).toUpperCase()}</span><div><strong>{userName}</strong><span>Signed in to demo account</span></div><button className="text-button" type="button" onClick={onLogout}>Log out</button></div> : <div className="signed-in-card"><div><strong>You are browsing as a guest</strong><span>Log in to see your past trips and saved details.</span></div></div>}<div className="profile-layout"><section className="panel preferences-panel"><PreferenceRow title={copy.easyMode} description={copy.easyDescription}><button className={`toggle ${easyMode ? 'on' : ''}`} type="button" aria-pressed={easyMode} aria-label="Toggle Easy Mode" onClick={onEasyMode}><i /></button></PreferenceRow><PreferenceRow title="Dark mode" description="A darker, low-glare view for evenings"><button className={`toggle ${darkMode ? 'on' : ''}`} type="button" aria-pressed={darkMode} aria-label="Toggle Dark Mode" onClick={onDarkMode}><i /></button></PreferenceRow><PreferenceRow title={copy.language} description={copy.languageDescription}><div className="language-buttons" role="group" aria-label={copy.language}>{languageOptions.map((option) => <button className={language === option.id ? 'selected' : ''} type="button" key={option.id} onClick={() => onLanguage(option.id)}>{option.label}</button>)}</div></PreferenceRow><PreferenceRow title={copy.savedPassengers} description={userName ?? 'Guest'}><button className="text-button" type="button">{copy.manage}</button></PreferenceRow></section><section className="panel about-panel"><div className="about-symbol"><Languages size={21} aria-hidden="true" /></div><h2>{copy.about}</h2><p>{copy.aboutText}</p><div className="trust-box"><ShieldCheck size={18} aria-hidden="true" /><div><strong>{copy.trustFirst}</strong><span>{copy.trustText}</span></div></div></section></div>{userName && <><section className="panel past-trips"><div className="panel-heading"><div><h2>Past trips</h2><p>Your completed demo journeys and their details.</p></div><Ticket size={20} aria-hidden="true" /></div>{pastTrips.map((trip) => <details className="past-trip" key={trip.id}><summary><span><strong>{trip.route}</strong><small>{trip.date} · {trip.status}</small></span><ChevronRight size={17} aria-hidden="true" /></summary><div className="past-trip-details"><SummaryRow label="Train" value={trip.train} /><SummaryRow label="Class" value={trip.className} /><SummaryRow label="Fare" value={formatCurrency(trip.fare)} /><SummaryRow label="Journey status" value={trip.status} /></div></details>)}</section><section className="panel service-history"><div className="panel-heading"><div><h2>My service requests</h2><p>Track your demo support requests here.</p></div><LifeBuoy size={20} aria-hidden="true" /></div>{serviceRequests.length ? serviceRequests.slice(0, 5).map((request) => <div className="service-history-row" key={request.reference}><div><strong>{request.serviceId.replaceAll('-', ' ')}</strong><span>{request.reference}</span></div><em>{request.status.replace('-', ' ')}</em></div>) : <p>No service requests yet.</p>}</section></>}</>
}
